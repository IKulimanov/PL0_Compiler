# PL/0 Grammar translation and P-code code generation
import Symbol
import Define
import Scanner
import copy
...
'''
Attribute class
-Define and save variables used at compile time
...
class PL0:
    ...
    #Return to PCode List object
    def getPCode(self):
        return self.parser.getPCode()

    #Return the converted PCode String
...
"""
BaseTool tool class
-Define some methods commonly used by compilers
"""
class BaseTool:
    def __init__(self):
        self.attr = Attribute()
    ...
    # errror (self, n)-report errors and log
    # Note: n is the error number, defined in Define
    def error(self,n):
        self.attr.errors +=1
        if n!=0:
    ...
    #List codes and return codes of String
    ...
    #Output symbol table from 0 to tx part
...
class Parser:
    ...
    #Return the generated PCode
    def getPCode(self):
        return self.attr.code

    #Back test skip reading
    ...
    def enter(self,k):
        ...
        #Return the position of id in the symbol table, or 0 if the symbol table does not exist
    ...
    def term(self, fsys):
        mulop = ''
        self.factor(fsys | set(['mul','div', 'mod']))
        while self.attr.sym in {'mul','div', 'mod'}:
            mulop = self.attr.sym
            self.tool.getSym()
            self.factor(fsys | {'mul','div', 'mod'})
            if mulop == "mul":
                self.tool.gen(Define.pCode['OPR'],0,4)
            elif mulop == "div":
                self.tool.gen(Define.pCode['OPR'],0,5)
            elif mulop == "mod":
                self.tool.gen(Define.pCode['OPR'], 0, 20)
